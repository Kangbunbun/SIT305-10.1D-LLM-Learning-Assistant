...
class TaskFragment : Fragment() {
    ...
    private val savedHintQuestionIds = mutableSetOf<Int>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvTaskTitle.text = currentTask.title
        binding.tvTaskTopic.text = "Topic: ${currentTask.topic}"

        setupQuestions(currentTask)
        setupSubmitButton(currentTask)
        observeViewModel()
    }
    ...
    private fun setupSubmitButton(task: Task) {
        binding.btnSubmit.setOnClickListener {
            if (userAnswers.size == task.questions.size) {
                ...
                userPrefs.addQuizResult(
                    totalQuestions = results.totalQuestions,
                    correctAnswers = results.score
                )

                saveQuestionResultsToHistory(results)
            ...
            states.forEach { (questionId, state) ->
                ...
                when (state) {
                    ...
                    is UIState.Success -> {
                        ...
                        saveHintToHistoryIfNeeded(
                            questionId = questionId,
                            prompt = state.data.prompt,
                            aiResponse = state.data.response
                        )
                    }
    ...
    private fun saveHintToHistoryIfNeeded(
        questionId: Int,
        prompt: String,
        aiResponse: String
    ) {
        if (savedHintQuestionIds.contains(questionId)) {
            return
        }

        val question = currentTask.questions.find { it.id == questionId } ?: return

        historyPrefs.saveOrUpdateHint(
        ...
        savedHintQuestionIds.add(questionId)
    }

    private fun saveQuestionResultsToHistory(results: TaskResult) {
        val answeredAt = System.currentTimeMillis()

        results.questionResults.forEach { questionResult ->
            historyPrefs.saveOrUpdateQuestionResult(
                topic = results.topic,
                question = questionResult.question.text,
                userAnswer = questionResult.userAnswer,
                correctAnswer = questionResult.question.correctAnswer,
