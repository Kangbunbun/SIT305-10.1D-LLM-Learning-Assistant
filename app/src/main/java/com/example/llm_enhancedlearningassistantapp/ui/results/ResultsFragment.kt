...
class ResultsFragment : Fragment() {
    ...
    private lateinit var currentResult: TaskResult

    private val savedExplanationQuestionIds = mutableSetOf<Int>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        currentResult = args.result

        binding.tvScore.text = "Score: ${currentResult.score}/${currentResult.totalQuestions}"

        setupResults(currentResult)
        setupDoneButton()
        observeViewModel()
    }
    ...
    private fun setupResults(result: TaskResult) {
        ...
        savedExplanationQuestionIds.clear()
            ...
            states.forEach { (questionId, state) ->
                ...
                when (state) {
                    ...
                    is UIState.Success -> {
                        ...
                        saveExplanationToHistoryIfNeeded(
    ...
    private fun saveExplanationToHistoryIfNeeded(
        questionId: Int,
        prompt: String,
        aiResponse: String
    ) {
        if (savedExplanationQuestionIds.contains(questionId)) {
            return
        }

        val questionResult: QuestionResult =
            currentResult.questionResults.find { it.question.id == questionId } ?: return

        historyPrefs.saveOrUpdateExplanation(
        ...
        savedExplanationQuestionIds.add(questionId)
    }
